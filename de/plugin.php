...
/**
 * Strings for component 'plugin', language 'de', branch 'MOODLE_25_STABLE'
 *
 * @package   plugin
...
$string['notwritable_help'] = 'Sie haben die automatische Updates fürs Plugins aktiviert. Das automatische Update kann aber nicht ausgeführt werden, weil das Plugin-Verzeichnis auf dem Server schreibgeschützt ist.

Sie müssen für das Plugin-Verzeichnis die Rechte so anpassen, dass der Webserver dort die notwendigen Dateien speichern darf. Anschließend können automatische Updates ausgeführt werden.';
...
$string['pluginchecknotice'] = 'Diese Seite zeigt Plugins, die Ihre Aufmerksamkeit erfordern. Markiert sind neue Plugins, aktualisierbare Plugins und fehlende Plugins. Die aktualisierbaren und fehlenden Versionen sollten Sie laden, bevor Sie die Aktualisierung von Moodle fortsetzen.';
...
$string['uninstallconfirm'] = 'Möchten Sie das Plugin \'{$a->name}\' wirklich löschen? Gleichzeitig werden alle mit dem Plugin verbundenen Daten aus der Datenbank gelöscht, die Konfiguration, alle Logdaten und Nutzerdaten. Diese Daten gehen verloren und Moodle bietet keine Möglichkeit zur Wiederherstellung an.';
...
$string['updatepluginconfirmexternal'] = 'Die derzeitige Version des Plugins wurde anscheinend vom Programmcode-Verwaltungssystem {$a} bezogen. Wenn Sie dieses Update installieren, können Sie das Verwaltungssystem für dieses Plugin nicht mehr verwenden. Soll dieses Update wirklich installiert werden?';
$string['updatepluginconfirminfo'] = 'Möchten Sie wirklich eine neue Version des Plugins <strong>{$a->name}</strong> installieren. Das ZIP-Paket des Plugins mit der Version {$a->version} wird heruntergeladen und entpackt. Anschließend wird das Plugin installiert.
<br />Quelle: <a href="{$a->url}">{$a->url}</a>';
